...
function AutoScroll(userOptions = {}) {
  ...
  function destroy() {
    if (!pluginIsActive()) return;
    emblaApi.off('pointerdown', onInteraction).off('pointerup', onInteraction).off('slidefocus', onInteraction).off('settle', settle);
    stopAutoScroll();
    destroyed = true;
  ...
  function createAutoScrollBehaviour(engine) {
    ...
    function seek() {
      ...
      const reachedEnd = options.direction === 'forward' ? pastMinBound(offsetLocation) : pastMaxBound(offsetLocation);
      if (!loop && reachedEnd) {
        hasSettled = true;
