'use client';
import { jsx, jsxs, Fragment } from 'react/jsx-runtime';
import * as React from 'react';
import { mergeProps } from '@base-ui/react/merge-props';
import { useRender } from '@base-ui/react/use-render';
import { cva } from 'class-variance-authority';
import { LazyMotion, domAnimation, AnimatePresence, m } from 'motion/react';
import { cn, focusableProps } from '../../internal/utils.js';
import { useReducedMotion } from '../../hooks/use-reduced-motion.js';
import { buttonVariants } from '../button/button-variants.js';

const chipSizeVariants = cva("", {
  variants: {
    size: {
      sm: "h-6 gap-0.75 rounded-xs px-2 text-xs has-data-[icon=end]:pe-1.5 has-data-[icon=start]:ps-1.5 [&_svg:not([class*='size-'])]:size-3.5",
      md: "h-8 gap-1 rounded-sm px-3 text-sm has-data-[icon=end]:pe-2 has-data-[icon=start]:ps-2 [&_svg:not([class*='size-'])]:size-4",
      lg: "h-10 gap-1.25 rounded-md px-3.5 text-base has-data-[icon=end]:pe-2.5 has-data-[icon=start]:ps-2.5 [&_svg:not([class*='size-'])]:size-4.5"
    }
  },
  defaultVariants: { size: "md" }
});
const ChipGroupContext = React.createContext(null);
const exitDefault = {
  opacity: 0,
  scale: 0.85,
  filter: "blur(8px)"
};
const exitReduced = { opacity: 0 };
const transition = { duration: 0.28, ease: [0.4, 0, 0.2, 1] };
const reducedTransition = { duration: 0 };
const closeIconSize = {
  sm: "size-3",
  md: "size-3.5",
  lg: "size-4"
};
function Chip({
  className,
  variant,
  size,
  render,
  dismissible = false,
  open,
  onOpenChange,
  onDismiss,
  closeLabel = "Dismiss",
  children,
  onClick,
  ...props
}) {
  const group = React.useContext(ChipGroupContext);
  const resolvedVariant = variant ?? group?.variant ?? "soft";
  const resolvedSize = size ?? group?.size ?? "md";
  const reduced = useReducedMotion();
  const [internalOpen, setInternalOpen] = React.useState(true);
  const isControlled = open !== void 0;
  const actualOpen = isControlled ? open : internalOpen;
  const triggerDismiss = React.useCallback(() => {
    if (!isControlled) setInternalOpen(false);
    onOpenChange?.(false);
  }, [isControlled, onOpenChange]);
  React.useEffect(() => {
    if (!group || !dismissible) return;
    return group.register(triggerDismiss);
  }, [group, dismissible, triggerDismiss]);
  const handleClick = (event) => {
    onClick?.(event);
    if (event.defaultPrevented) return;
    if (dismissible) triggerDismiss();
  };
  const composedChildren = dismissible ? /* @__PURE__ */ jsxs(Fragment, { children: [
    children,
    /* @__PURE__ */ jsx(ChipCloseIcon, { size: resolvedSize }),
    /* @__PURE__ */ jsx("span", { className: "sr-only", children: closeLabel })
  ] }) : children;
  const chipElement = useRender({
    defaultTagName: "button",
    render,
    state: { variant: resolvedVariant, size: resolvedSize, dismissible },
    props: mergeProps(
      {
        "data-slot": "chip",
        "data-dismissible": dismissible || void 0,
        type: render ? void 0 : "button",
        ...focusableProps(props.disabled),
        className: cn(
          buttonVariants({ variant: resolvedVariant }),
          chipSizeVariants({ size: resolvedSize }),
          dismissible && "group/chip",
          className
        ),
        onClick: handleClick,
        children: composedChildren
      },
      props
    )
  });
  if (!dismissible) return chipElement;
  return /* @__PURE__ */ jsx(LazyMotion, { features: domAnimation, strict: true, children: /* @__PURE__ */ jsx(AnimatePresence, { initial: false, onExitComplete: () => onDismiss?.(), children: actualOpen && /* @__PURE__ */ jsx(
    m.span,
    {
      initial: false,
      exit: reduced ? exitReduced : exitDefault,
      transition: reduced ? reducedTransition : transition,
      className: "inline-flex align-middle",
      children: chipElement
    },
    "chip"
  ) }) });
}
function ChipCloseIcon({ size }) {
  return /* @__PURE__ */ jsx(
    "svg",
    {
      xmlns: "http://www.w3.org/2000/svg",
      viewBox: "0 0 16 16",
      fill: "currentColor",
      "aria-hidden": "true",
      "data-slot": "chip-close-icon",
      "data-icon": "end",
      className: cn(
        closeIconSize[size],
        "opacity-60 transition-opacity duration-200 group-hover/chip:opacity-100 motion-reduce:transition-none"
      ),
      children: /* @__PURE__ */ jsx("path", { d: "M11.523 3.522c.264-.264.691-.264.955 0s.264.691 0 .955L8.955 8l3.522 3.522c.264.264.264.691 0 .955s-.691.264-.955 0L8 8.955l-3.522 3.522c-.264.264-.691.264-.955 0s-.264-.691 0-.955L7.045 8 3.522 4.478c-.264-.264-.264-.691 0-.955s.691-.264.955 0L8 7.045l3.523-3.522z" })
    }
  );
}
function ChipGroup({ ref, className, variant, size, children, ...props }) {
  const dismissersRef = React.useRef(null);
  if (dismissersRef.current === null) {
    dismissersRef.current = /* @__PURE__ */ new Set();
  }
  React.useImperativeHandle(
    ref,
    () => ({
      clearAll: () => {
        dismissersRef.current.forEach((dismiss) => dismiss());
      }
    }),
    []
  );
  const contextValue = React.useMemo(
    () => ({
      register: (dismiss) => {
        dismissersRef.current.add(dismiss);
        return () => {
          dismissersRef.current.delete(dismiss);
        };
      },
      variant,
      size
    }),
    [variant, size]
  );
  return /* @__PURE__ */ jsx(ChipGroupContext.Provider, { value: contextValue, children: /* @__PURE__ */ jsx("div", { "data-slot": "chip-group", className: cn("flex flex-wrap items-center gap-2", className), ...props, children }) });
}

export { Chip, ChipGroup };
//# sourceMappingURL=chip.js.map
