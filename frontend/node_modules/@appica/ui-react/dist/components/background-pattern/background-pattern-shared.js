...
function svgMask(body, width = 12, height = width) {
...
const PATTERN_MASKS = {
  ...
  grid: {
    maskImage: "linear-gradient(to right, #000 0 1px, transparent 1px), linear-gradient(to bottom, #000 0 1px, transparent 1px)",
    cell: 28,
    tint: "bg-current/7",
...
function patternCell(variant) {
  return PATTERN_MASKS[variant].cell;
}
function patternTint(variant) {
  return PATTERN_MASKS[variant].tint;
}
function patternSpotlight(variant) {
  return PATTERN_MASKS[variant].spotlight;
}
function PatternLayer({ variant, className }) {
...
export { PatternLayer, patternCell, patternSpotlight, patternTint };
//# sourceMappingURL=background-pattern-shared.js.map
