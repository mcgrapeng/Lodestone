...
import { format, formatISO, isValid, getDaysInMonth } from 'date-fns';
...
const EMPTY_PARTS = { year: null, month: null, day: null };
...
const MONTH_SHORT = Array.from({ length: 12 }, (_, i) => format(new Date(2020, i, 1), "MMM"));
const MONTH_LONG = Array.from({ length: 12 }, (_, i) => format(new Date(2020, i, 1), "MMMM"));
function parseFormat(formatStr) {
  ...
  while ((match = re.exec(formatStr)) !== null) {
    ...
    const token = match[0];
    const type = token[0] === "M" ? "month" : token[0] === "y" ? "year" : "day";
...
function displayPart(part, token) {
...
function DateField({
  ...
  const [internalParts, setInternalParts] = React.useState(() => partsFromDate(value ?? defaultValue));
  const lastValueIsoRef = React.useRef(safeIso(value));
  const lastCommittedRef = React.useRef(partsToDate(partsFromDate(value ?? defaultValue))?.toISOString() ?? "");
  React.useEffect(() => {
  ...
  const typingCountRef = React.useRef(/* @__PURE__ */ new Map());
  ...
  const focusSegment = React.useCallback((idx) => {
  ...
  const onRootMouseDown = React.useCallback(
    (e) => {
      ...
      const firstEmpty = nodes.find(
        (n) => n.kind === "segment" && getPartFor(n.type, partsRef.current) == null
      );
  ...
  const onSegmentFocus = React.useCallback((seg) => {
    typingCountRef.current.delete(seg.index);
  }, []);
  const onSegmentBlur = React.useCallback((seg) => {
    typingCountRef.current.delete(seg.index);
  }, []);
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      ref,
      className: cn(
        unstyled ? "inline-flex min-w-0 items-center select-none data-disabled:pointer-events-none data-disabled:cursor-not-allowed" : cn(
      ...
      children: [
        startSlot ? /* @__PURE__ */ jsx("div", { "data-slot": "date-field-start", className: "-ms-1 shrink-0", children: startSlot }) : null,
        /* @__PURE__ */ jsx("div", { "data-slot": "date-field-segments", className: "text-foreground flex min-w-0 flex-1 items-center", children: nodes.map((node, i) => {
          if (node.kind === "literal") {
            return /* @__PURE__ */ jsx("span", { "aria-hidden": "true", className: "text-foreground-subtle whitespace-pre", children: node.text }, `l-${i}`);
          }
          ...
          return /* @__PURE__ */ jsx(
            "span",
            {
              ...
              children: displayPart(partVal, node.token)
            },
            `s-${node.index}`
          );
        }) }),
        endSlot ? /* @__PURE__ */ jsx("div", { "data-slot": "date-field-end", className: "-me-1 shrink-0", children: endSlot }) : null,
        name ? /* @__PURE__ */ jsx("input", { type: "hidden", name, value: hiddenValue, required, disabled }) : null
      ]
    }
  );
}
