...
function Fade(userOptions = {}) {
  ...
  function init(emblaApiInstance, optionsHandler) {
    ...
    opacities = emblaApi.snapList().map((_, index) => index === selectedSnap ? fullOpacity : noOpacity);
  ...
  function getFadeIndex() {
    ...
    const nextSnap = indexCurrent.clone().set(selectedSnap).add(directionSign * -1).get();
  ...
  function createFadeSsrStyles(options, containerSelector, slidesSelector) {
    const {
      active
    } = options;
    const {
    ...
    const slideStyles = slidesBySnap[selectedSnap].reduce((styles, index) => {
      return styles + `${containerSelector} ${slidesSelector}:nth-child(${index + 1}){opacity:1;pointer-events:auto;}`;
    }, '');
    if (active) return baseStyles + slideStyles;
    return baseStyles;
  }
  function ssrStyles(containerSelector, slidesSelector = '> *') {
    if (!isSsr) return '';
    const optionBreakpoints = userOptions.breakpoints || {};
    const baseStyles = createFadeSsrStyles(options, containerSelector, slidesSelector);
    const mediaStyles = Object.keys(optionBreakpoints).reduce((styles, key) => {
      return styles + `@media ${key}{${createFadeSsrStyles(optionBreakpoints[key], containerSelector, slidesSelector)}}`;
    }, '');
