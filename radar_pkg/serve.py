        ...
        def do_POST(self):
            ...
            if self.path == "/api/install_to_target":
                try:
                    ...
                    self._json({"ok": False, "error": str(e)}, status=400)
                return
            # ponytail: 2026-09 FU-marquee — 批量安装/卸载分类下所有 skill。
            # 前端 CategoryBrowser "一键安装全部" / "一键卸载全部" 触发。
            # handler 必须秒返(不阻塞 HTTP),30s clone × N repos 走 daemon thread,
            # 前端轮询 /api/install/status 看进度。
            if self.path == "/api/install_category":
                try:
                    body = self._read_body()
                    cat_id = (body.get("category_id") or "").strip()
                    target = (body.get("target") or "").strip()
                    if not cat_id or target not in SUPPORTED_CLIS:
                        return self._json(
                            {
                                "error": "category_id + target (one of "
                                + ", ".join(SUPPORTED_CLIS)
                                + ") required"
                            },
                            status=400,
                        )
                    except_ = set(body.get("except") or [])
                    _latest = DATA / "latest.json"
                    if not _latest.exists():
                        return self._json(
                            {"error": "no data — run ./radar.py crawl first"},
                            status=503,
                        )
                    _snap = json.loads(_latest.read_text())
                    _cat_data = [
                        c for c in (_snap.get("categories") or []) if c.get("id") == cat_id
                    ]
                    if not _cat_data:
                        return self._json(
                            {"error": f"unknown category: {cat_id}"}, status=400
                        )
                    _cat = _cat_data[0]
                    _repos = [
                        r
                        for r in _cat.get("repos", [])
                        if r.get("name") not in except_ and r.get("is_skill")
                    ]
                    _total = len(_repos)
                    ...
                    def _do_install_cat():
                        ok = err = 0
                        for idx, repo in enumerate(_repos, 1):
                            try:
                                _core_local.write_install_status(
                                    computing=True,
                                    current_index=idx,
                                    total=_total,
                                    current=f"install {repo['name']}",
                                    error=None,
                                )
                                install_skill_from_github(
                                    repo["name"], repo["url"], targets=[target]
                                )
                                ok += 1
                            except Exception as e:
                                err += 1
                                _core_local.write_install_status(
                                    computing=True,
                                    current=f"install {repo['name']} FAILED",
                                    error=f"{type(e).__name__}: {e}",
                                    error_count=err,
                                )
                        _core_local.write_install_status(
                            computing=False, error_count=err
                        )
                        invalidate_local_scan()

                    threading.Thread(
                        target=_do_install_cat,
                        daemon=True,
                    ...
                    return self._json({"error": str(e)}, status=500)
            if self.path == "/api/uninstall_category":
                try:
                    body = self._read_body()
                    cat_id = (body.get("category_id") or "").strip()
                    target = (body.get("target") or "").strip()
                        ...
                        for idx, repo in enumerate(_repos, 1):
                            try:
                                _core_local.write_install_status(
                                    ...
                                    current_index=idx,
                                    total=_total,
                                    current=f"uninstall {repo['name']}",
                                    error=None,
                                )
                                uninstall_skill(repo["name"], targets=[target])
                                ok += 1
                            except Exception as e:
                                err += 1
                                _core_local.write_install_status(
                                    computing=True,
                                    current=f"uninstall {repo['name']} FAILED",
                                    error=f"{type(e).__name__}: {e}",
                                    error_count=err,
                                )
                        _core_local.write_install_status(
                    ...
                    threading.Thread(
                        target=_do_uninstall_cat,
                        daemon=True,
                        name="uninstall_category",
                    ).start()
                    return self._json(
                        {
                            "ok": True,
